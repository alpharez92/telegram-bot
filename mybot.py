...
myDb=mysql.connector.connect(host='localhost',user='root',database='db_starbhak')
sql=myDb.cursor()
...
class Mybot:
    ...
    def start(message):
        ...
        teks = mytoken.SAPA + "\n-- admin & developer @RizkiAlfaReza - SMK Taruna Bhakti -- "+"\n" \
    ...
    def menu_data_siswa(message):
        query="select id,nama_siswa,jurusan from tbl_siswa "
        sql.execute(query)
        data=sql.fetchall()
